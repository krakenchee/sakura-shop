...
if (isset($_GET['delete'])) {
    ...
    $st = $db->prepare("SELECT image FROM categories WHERE id = ?");
    $st->execute([$id]);
    $cat = $st->fetch();
    if ($cat && $cat['image']) {
        $filePath = dirname(__DIR__) . '/' . $cat['image'];
    ...
    if (!$name || !$mainCategoryId) {
        ...
        if ($checkSlug->fetch()) {
            ...
            if ($id && $image) {
                ...
                $st->execute([$id]);
                $oldCat = $st->fetch();
                if ($oldCat && $oldCat['image'] && $oldCat['image'] !== $image) {
                    $oldFilePath = dirname(__DIR__) . '/' . $oldCat['image'];
...
<div class="admin-layout">
                ...
                <div class="admin-form-grid">
                    <div class="admin-form-group">
                        ...
                        <input type="text" class="admin-form-input" name="name" id="catName" required
                               value="<?= htmlspecialchars($editSubcat['name'] ?? '') ?>"
                               oninput="document.getElementById('slugPreview').value = generateSlugFromName(this.value)">
                    </div>
                    ...
                    <div class="admin-form-group">
                        ...
                        <small class="admin-form-help">Человеко-понятный URL. Будет сгенерирован автоматически из названия</small>
                    </div>
                    ...
                    <div class="admin-form-group">
                        <label class="admin-form-label">Изображение категории</label>
                        <div class="image-uploader" data-type="category">
                            <div class="upload-area" onclick="document.getElementById('categoryImageInput').click()">
                                <input type="file" id="categoryImageInput" accept="image/*" style="display:none">
                                <div class="upload-placeholder" <?= ($editSubcat && $editSubcat['image']) ? 'style="display:none"' : '' ?>>
                                    <span class="upload-icon">📸</span>
                                    <p>Нажмите для загрузки изображения</p>
                                    <small>JPG, PNG, WEBP до 5MB</small>
                                </div>
                                <div class="upload-preview" <?= ($editSubcat && $editSubcat['image']) ? '' : 'style="display:none"' ?>>
                                    <img src="<?= ($editSubcat && $editSubcat['image']) ? BASE_URL . $editSubcat['image'] : '' ?>" alt="Preview">
                                    <button type="button" class="remove-image-btn">✕</button>
                                </div>
                            </div>
                            <input type="hidden" name="image" id="imagePathInput" value="<?= htmlspecialchars($editSubcat['image'] ?? '') ?>">
                        ...
                        <thead>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?php if ($subcat['image']): ?>
                                        <img src="<?= BASE_URL . $subcat['image'] ?>" style="width: 40px; height: 40px; object-fit: cover; border-radius: 4px;">
                                        <?php else: ?>
    ...
    const categoryFileInput = document.getElementById('categoryImageInput');
    if (categoryFileInput) {
        categoryFileInput.addEventListener('change', async function(e) {
