...
// Получить текущую категорию (теперь из main_categories)
$currentCat = null;
if ($slug) {
    $st = $db->prepare("SELECT * FROM main_categories WHERE slug = ?");
    $st->execute([$slug]);
    $currentCat = $st->fetch();
}

// Все основные категории
$topCats = $db->query("SELECT * FROM main_categories ORDER BY id ASC")->fetchAll();

// Подкатегории текущей категории (из таблицы categories)
$subCats = [];
if ($currentCat) {
    $st = $db->prepare("SELECT * FROM categories WHERE main_category_id = ? ORDER BY name ASC");
...
if ($currentCat) {
    ...
    if ($subSlug) {
        $st = $db->prepare("SELECT id FROM categories WHERE slug = ? AND main_category_id = ?");
        $st->execute([$subSlug, $currentCat['id']]);
        $sub = $st->fetch();
        if ($sub) $categoryIds = [$sub['id']];
    }
    // Если подкатегорий нет или не выбраны — все товары в подкатегориях этой основной категории
    if (!$categoryIds) {
...
switch ($sort) {
    ...
    case 'new':
        ...
        break;
    default:
        $sortSQL = 'p.is_popular DESC, p.order_count_30d DESC, p.created_at DESC';
        break;
}
...
if ($currentCat) {
    ...
    $ogImage = $currentCat['image'] ?? BASE_URL . 'assets/images/og-image.jpg';
} else {
    ...
    <aside class="catalog-sidebar">
      ...
      <div class="filter-card">
        ...
        <div class="subcategory-list">
          <a href="catalog.php?cat=<?= urlencode($slug) ?>" class="subcategory-btn <?= !$subSlug ? 'active' : '' ?>">
            Все
          </a>
          <?php foreach ($subCats as $sc): ?>
          <a href="catalog.php?cat=<?= urlencode($slug) ?>&sub=<?= urlencode($sc['slug']) ?>"
             class="subcategory-btn <?= $subSlug === $sc['slug'] ? 'active' : '' ?>">
