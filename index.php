...
// Активные баннеры
$banners = $db->query("SELECT * FROM banners WHERE is_active = 1 ORDER BY id DESC")->fetchAll();
...
$popular = $db->query("
    ...
    ORDER BY p.order_count_30d DESC, p.created_at DESC LIMIT 8
")->fetchAll();

// Если нет товаров с is_popular=1, показываем товары с наибольшим количеством заказов
if (empty($popular)) {
    $popular = $db->query("
        SELECT p.*, pi.image_path, c.name as cat_name
        FROM products p
...
// Если нет товаров с is_new=1, показываем последние добавленные
...
// Основные категории
$categories = $db->query("SELECT * FROM main_categories ORDER BY id ASC")->fetchAll();
